...
module.exports = async (req, res) => {
  ...
  const to   = process.env.CONTACT_TO_EMAIL   || "certifiedtransmission@rocketmail.com";
  const from = process.env.CONTACT_FROM_EMAIL || "Certified Transmission of Ball <leads@certifiedtransmissionofball.com>";

  const subject = `New lead from certifiedtransmissionofball.com — ${name}`;
  const text =
`Name: ${name}
Email: ${email}
Phone: ${phone}
...
—
Sent from the contact form on certifiedtransmissionofball.com`;
  const html = `
<h2>New lead from the Certified Transmission of Ball site</h2>
...
<p style="color:#666;font-size:13px">Sent from the contact form on certifiedtransmissionofball.com</p>`;
